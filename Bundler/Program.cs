...
namespace Bundler
{
    class Program
    {
        public const string ProjName = "DataBaseDPD";
        public const string inBaseRelPath = @"..\..\..\";
        ...
        public static void Main(string[] args)
        {
            ...
            string version;
            string mainExe = @"DBConsole\bin\Release\Console.exe";
            string mainExeRelPath = inBaseRelPath + mainExe;

            version = GetVersion(mainExeRelPath);

            outBaseFolder = ProjName + "-" + version + @"/"; //name of the folder created inside the zip

            files.Add(mainExeRelPath);
            files.Add(inBaseRelPath + @"DBManagementLib\bin\Release\DBManagementLib.dll");
            //If you want add any other files , you need to add files.Add(inBaseRealBath +....);
            ...
            string outputFile = inBaseRelPath + ProjName + "-" + version + ".zip";
                            ...
                            foreach (string file in files)
                                {
                                    if(System.IO.File.Exists(file))
                                        ...
                                        {
                                            Console.WriteLine("Couldn't find file:{0}",file);
                                        }
